...
public class ConstantApiTests
{
    ...
    [Fact]
    public async Task CreateConstant_ShouldReturnForbidden_WhenPassingMemberTokenInsteadOfDeveloperToken()
    {
        var createdResponse = await
            new HttpRequestBuilder(_client, HttpMethod.Post, SettingsEndpoints.ConstantEndpoints.CreateConstant)
    ...
    [Fact]
    public async Task UpdateConstant_ShouldReturnForbidden_WhenPassingMemberTokenInsteadOfDeveloperToken()
    {
        var createdResponse = await
            new HttpRequestBuilder(_client, HttpMethod.Put, $"{SettingsEndpoints.ConstantEndpoints.CreateConstant}/{Guid.NewGuid()}")
                .WithXApiKeyHeader(UsersDbSeeder.XApiKeyMember)
    ...
    [Fact]
    public async Task DeleteConstant_ShouldReturnForbidden_WhenPassingMemberTokenInsteadOfDeveloperToken()
    {
    ...
    public async Task CreateConstant_ShouldFail_WhenPassingNullValue()
    {
        ...
        createdResponse.responseMessage.StatusCode.ShouldBe(HttpStatusCode.UnprocessableEntity);
        createdResponse.responseContent!.GetType().ShouldBe(typeof(ApiErrorResponse));
        createdResponse.responseContent.Errors.First().Slug.ShouldBe(ConstantErrors.ValidationValueIsRequired.Code);
    }
    ...
    public async Task UpdateConstant_ShouldUFail_WhenPassingNullValue()
    {
        ...
        updatedResponse.responseMessage.StatusCode.ShouldBe(HttpStatusCode.UnprocessableEntity);
        updatedResponse.responseContent!.GetType().ShouldBe(typeof(ApiErrorResponse));
        updatedResponse.responseContent.Errors.First().Slug.ShouldBe(ConstantErrors.ValidationValueIsRequired.Code);
    }
