...
public class GetAccountByIdQueryHandler(UserManager<AppUser> userManager)
    : IRequestHandler<GetAccountByIdQuery, ErrorOr<AccountResponse>>
{
    public async Task<ErrorOr<AccountResponse>> Handle(GetAccountByIdQuery query, CancellationToken cancellationToken)
    {
        ...
        if (account is null) return AppUserErrors.AppUserWithIdDoesNotExist;
